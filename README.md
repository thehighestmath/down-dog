...
2. При установке Windows желательно включить **Add Python to PATH**.
...
Если проект использует переменные окружения, файл `.env` должен находиться в корне проекта:
...
### 10.1. Перейдите в папку проекта

```bash
cd путь_к_проекту
```

### 10.2. Активируйте виртуальное окружение
...
### 10.3. Запустите FastAPI

```bash
uvicorn app.main:app --reload
```

## 11. Если Uvicorn не запускается
...
## 12. Если появилась ошибка импорта
...
## 14. Порядок работы в проекте Down Dog
